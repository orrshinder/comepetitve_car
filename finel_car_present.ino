...
int couting = 0;
  ...
  int Dis = 0;
  int flag_state = 0;
  int flag_state_right = 0;
  int flag_state_left = 0;
  int count_wall = 0;
  int x = 25;
  int y = 30;
  int flag_wall=0;
...
void setup() {
  ...
  int count_wall = 0;
  int x = 25;
  sinal_360=digitalRead(turn_signal);
...
/*
 while(true){             
   ...
   while((millis()<(start_time+2085))&&(IRsensor_Front() < 1040)&&(sinal_360==0))  //first task: look for IR, and spin around
      {
        motors(53,53,BACKWARD,BACKWARD,FORWARD,FORWARD);
      }
      Stop_Motor();
      delay(100);
      sinal_360=0;
      ...
      while((ultra_front() > 28)&&(ultra_left() > x)&&(ultra_right() > x)){ // not seeing any obsicel moveing forword
        //Serial.print("distans in santimeter front :");
          // Serial.println(ultra_front());
          Serial.print("not seeing any obsicel moveing forword");
          Serial.println(); 
          motors(70,70,FORWARD,FORWARD,FORWARD,FORWARD);
        }
        Stop_Motor();
        delay(50);
      while((ultra_front() > x)&&(ultra_front() < 30)){
          Serial.print("near to obstical in front");
          Serial.println();
      ...
      while((ultra_front() < 25) && (ultra_right() > 25)&&(ultra_left() > 25)){ // seeing obstical in front, rotet left antill ultra right see less then 40 
        Serial.print("seeing obstical in front, rotet left antill ultra right see less then 25");
          Serial.println();
      if(flag_turn == 0)
      {
        motors(70,70,FORWARD,FORWARD,BACKWARD,BACKWARD);
      }
      else
      {
        motors(70,70,BACKWARD,BACKWARD,FORWARD,FORWARD);
      }
      }    
      Stop_Motor();
      delay(50); 
      ...
      while(ultra_left() < 25){ // obstical in left is wall
        Serial.print("obstical in left is wall");
          Serial.println(); 
        count_wall = 0;
        motors(50,50,BACKWARD,BACKWARD,FORWARD,FORWARD);
        sinal_360=0;
        flag_wall=1;
      }
      if(flag_wall==1)
      ...
      delay(50);
       ...
       if(flag_wall==1)
      {
              Serial.print("00000000000000000000000000000000000000000000000000000000ppppppppppppppppppppppppppppppppppppppppppppppppppppppssssssssssssssssssssssssssssss");
          Serial.println(); 
       motors(60,60,FORWARD,FORWARD,FORWARD,FORWARD);
      }
     flag_wall=0;
